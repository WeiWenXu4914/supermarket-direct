...
<template>
  <div class="orderListWrapper">
    ...
    <div class="zong" v-for="(item, index) in dataListItem" :key="index">
      <div class="content">
        <div class="list" @click="intoStore(item.entid)">
          ...
          <p class="store-name">
            {{ item.ent_name }} <van-icon class="arrow-icon" name="arrow" />
          </p>
          <p class="p2">{{ titleInfo(item.status) }}</p>
        </div>
      </div>
      <div class="wrapper" @click="toDetail(item)">
        <div class="goods">
          <img :src="item.pro_thumbnail" alt />
          <div class="center">
            <p class="p1">{{ item.pro_name }}</p>
            <p class="p2" v-if="item.dw_id == 3">包邮</p>
            <p class="p2" v-else>到店自取</p>
      ...
      <div class="allBtn" v-else-if="item.status == 6">
        <button type="default" @click="dellOrder(item)">删除订单</button>
        <button type="default" class="red" @click="toBuy(item)">
          进店购买
        </button>
      </div>
      <div class="allBtn" v-else-if="item.status == 5">
      ...
      <div class="allBtn" v-else-if="item.status == 1 && item.dw_id == 3">
        <button type="default" @click="toRefund(item)">申请退款</button>
        <button type="default" class="red" @click="urgeSent(item)">
          提醒发货
        </button>
      </div>
      <div class="allBtn" v-else-if="item.status == 1 && item.dw_id == 1">
    ...
    <van-empty
      description="无订单"
      class="state"
      v-if="dataListItem.length === 0 ? true : false"
    />
  </div>
</template>
  ...
  methods: {
    ...
    getDate() {
      ...
      orderList(obj)
        .then((res) => {
          ...
          //分类数据
          if (this.state === 0) {
            //已删除  is_del 1 是  0 否     待评价同理
            this.dataListItem = res.data.filter((item) => item.is_del != 1);
          } else if (this.state === 1) {
            //0 待付款；1待发货；2已发货；3已完成；4已关闭；5无效订单 6是过期 --->status
            this.dataListItem = res.data.filter(
              (item) => item.status == 0 && item.is_del != 1
            );
          } else if (this.state === 2) {
            this.dataListItem = res.data.filter(
              (item) => item.status == 1 && item.is_del != 1
            );
          } else if (this.state === 3) {
            this.dataListItem = res.data.filter(
              (item) => item.status == 2 && item.is_del != 1
            );
          } else {
            this.dataListItem = res.data.filter(
              (item) =>
                item.is_mark == 0 && item.status == 3 && item.is_del != 1
            );
          }
    ...
    cancelOrders(order) {
      Dialog.confirm({
        ...
        cancelOrder(order.order_number).then((res) => {
          if (res.code === 200) {
            Toast("取消订单成功");
            this.isCancel = false;
          }
    ...
    dellOrder(order) {
      Dialog.confirm({
        ...
        .then(() => {
          // on confirm
          delOrder(order.order_number).then((res) => {
            if (res.code === 100) {
              Toast("删除订单成功");
              this.page = 1;
              this.getDate();
      ...
      if (this.page > 2) {
        ...
        await orderList(obj)
          .then((res) => {
            ...
            if (this.state === 0) {
              //已删除  is_del 1 是  0 否     待评价同理
              this.dataListItem.push(
                ...res.data.filter((item) => item.is_del != 1)
              );
            } else if (this.state === 1) {
              //0 待付款；1待发货；2已发货；3已完成；4已关闭；5无效订单 6是过期 --->status
              this.dataListItem.push(
...
>>>>>>> f14358ed9380ac6c8cc23186c210bf1b08e3547b
</style>