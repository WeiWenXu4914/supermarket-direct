<template>
  <div class="obligation-not-payment">
    ...
    <div class="title-info" v-if="data.status == 0">
      ...
      <span
        >剩余：<van-count-down :time="time" format="DD 天 HH 时 mm 分 ss 秒"
      /></span>
    </div>
    ...
    <div class="order-info">
      ...
      <p v-if="data.order_paytime !== null">
        下单时间： {{ data.order_paytime }}
      </p>
    ...
    <div class="total-money">
      <p>
        ...
        ><span
          ...
          >{{ data.pro_price * parseInt(data.buy_num) }}</span
        >
      </p>
      ...
      <p>
        ...
        ><span
          >-{{
            data.order_discount == "0" ? "0.00" : data.order_discount
          }}</span
        >
      </p>
    </div>
    ...
    <div class="confirm-area">
      ...
      <div class="button" v-else-if="data.status == 3">
        ...
        <button @click="appraise(data)" v-if="data.is_mark == 0">
          评价商品
        </button>
      </div>
      <!--交易失败-->
...
export default {
  ...
  created() {
    ...
    searchMemberSite(this.data.mem_site_id).then((res) => {
      const data = res.data;
      if (data) {
        this.address =
          data.province + data.city + data.district + data.detailed_site;
      }
    });
  },
  methods: {
